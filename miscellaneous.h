...
#define errxit(MSG) fprintf(stderr,"error; %s; %s %d %s\n",MSG,__FILE__,__LINE__,__func__),exit(1);
...
//return a number up to which type within regex that compared true on string, ie, an index where each quantity is an increment counting a type that was chronologically specified within char *regex. P and Q can be left NULL; they're used for recursiveness (returns negative number if typecount is 100% matched)
