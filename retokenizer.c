...
/*
  external function which determines precedence and ambiguity
  eg a operator say + can be either INFIX or PREFIX depending on context
*/
...
Stoken *un_slash_ambiguity(Stoken *root/*affix*/)
{
 ...
 repeat_unambiguity:
  ...
  while(1)
    {
      if(token == NULL || (tt.NUMBER & ttn_eol)) // end of list ?
...
/* algorithm for R and L like ( and )
   ...
   NOTE; innermost solved first and leaves no R or L behind so a RE can safely scan for L (cuz R is passed to RE)
   NOTE; upon passing R to RE the inside of the R and L is guaranteed retokenized
*/
Stoken *randlrun(Stoken *token)
{
  Stoken *first_token = token;
  while(token){ //go to R
    if((tt.NUMBER & ttn_randl)
       && tt.randl.rorl % 2 == 0){
      int rorl; Stoken *R;
      rorl = tt.randl.rorl; R = token;
      while(token)
	{ // go to match of L to R
	  if((tt.NUMBER & ttn_randl)
	     && tt.randl.rorl % 2 != 0)
	    {
  ...
  return first_token; // this ought to be a token like ttn_eol at beginning of ll
}
...
Stoken *retokenize(Stoken *token)
{
  ...
  { //retokenize remaining affixes (important!) no R and L enclosing required
    Stoken *tmp = token;
    while(token){
      if(tt.NUMBER
	 & ttn_affix)
